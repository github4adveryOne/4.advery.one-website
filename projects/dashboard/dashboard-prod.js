...
class AgentDashboard {
    ...
    connectWebSocket() {
        ...
        const token = localStorage.getItem('agent_dashboard_session');
        if (!token) {
            console.log('No session token found, showing auth overlay');
        ...
        this.socket.on('init', (data) => {
            this.handleWebSocketMessage({ type: 'init', ...(data || {}) });
        });

        this.socket.on('agent_update', (data) => {
            this.handleWebSocketMessage({ type: 'agent_update', ...(data || {}) });
        });
...
class AuthManager {
    constructor(dashboard) {
        ...
        this.apiBase = '/dashboard-api/api';
        this.identifier = '';
    ...
    setupAuthListeners() {
        const reqBtn = document.getElementById('requestOtpBtn');
        ...
        reqBtn.addEventListener('click', () => this.requestOTP());
        ...
        // Allow Enter key
        identifierInput.addEventListener('keypress', (e) => { if(e.key === 'Enter') this.requestOTP(); });
        otpInput.addEventListener('keypress', (e) => { if(e.key === 'Enter') this.verifyOTP(); });
    }

    async requestOTP() {
        this.identifier = document.getElementById('userIdentifier').value.trim();
        if (!this.identifier) return this.showMessage('Please enter an identifier', 'error');

        this.showMessage('Sending OTP...', 'info');
        try {
            const response = await fetch(`${this.apiBase}/auth/request-otp`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ user_identifier: this.identifier })
            });
            ...
            if (data.success) {
                ...
                this.showMessage('Check your Telegram!', 'success');
            } else {
                this.showMessage(data.error || 'Failed to request OTP', 'error');
            }
        } catch (err) {
    ...
    async verifyOTP() {
        ...
        if (!otp) return this.showMessage('Enter the code', 'error');
        ...
        try {
            const response = await fetch(`${this.apiBase}/auth/verify-otp`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ user_identifier: this.identifier, otp: otp })
            });
            const data = await response.json();
            
            if (data.success) {
                localStorage.setItem('agent_dashboard_session', data.session_token);
                this.enterDashboard();
            } else {
    ...
    async checkExistingSession() {
        ...
        try {
            ...
            if (data.success) {
                ...
                localStorage.removeItem('agent_dashboard_session');
            }
        } catch (err) {
    ...
    enterDashboard() {
        ...
        this.dashboard.connectWebSocket();
    }

    showMessage(msg, type) {
        const el = document.getElementById('authMessage');
        el.textContent = msg;
        el.style.color = type === 'error' ? '#ff5555' : (type === 'success' ? '#00adb5' : '#ffffff');
    }
}

window.addEventListener('DOMContentLoaded', () => {
    window.dashboard = new AgentDashboard();
    window.auth = new AuthManager(window.dashboard);
});
